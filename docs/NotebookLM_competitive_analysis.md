...
> **Shipment status (updated Aug 2026):** since this analysis was written, NorAI has shipped the top adoption items — **A (Socratic Study mode) and F (custom tutor persona)** are live in the tutor (`ChatState.study_mode` + `persona_instructions`, `SOCRATIC_SYSTEM_PROMPT`, Study segment + `PersonaModal` in the AI panel), **D (per-chapter concept/mind map)** is live in the workspace (Phase D concept map, static-SVG export), and **B (study-session persistence)** is fully shipped: flashcard ratings + a true SM-2 scheduler + Missed-filter deck + Anki `.apkg` export, quiz attempt history + deterministic missed-question review (P6.2 QA pass 2026-08-12). The **moat work is now largely shipped too (P6.3, 2026-08-13)**: an embedded YouTube player (floating "Watch video" → docked player) with **chapter-level click-to-seek** from sidebar rows, NotesView headings, and tutor references (chapter-level best-effort; exact-moment chunk seeks remain a follow-up, P6.4c). Remaining open items are C/E (difficulty/quantity knobs, explain-with-citation, Study Guide report). Marked with **✅ shipped** below.
...
**B. Study-session persistence for flashcards + quizzes ("Got it/Missed it", review-missed-only, results history)** — *Low-Medium, high value.* **✅ shipped** — flashcard ratings persist per lecture with an SM-2 schedule (lazy `easiness`/`reps`/`interval_days`/`due_at`/`last_reviewed_at` columns on the `flashcard_ratings` table, `POST /flashcards/ratings` upsert), the Cards panel has a Missed (Again/Hard) deck filter + deck-scoped stats, Anki `.apkg` export ships, and quiz attempts persist with a history UI (`GET /quiz/attempts`) + deterministic missed-question review (`GET /quiz/attempts/{id}/missed`, `_compute_missed_ids`). Only the deck-level stats' historical review (session history per card) is still ahead.
Maps onto: ratings/scores persist per lecture in the per-lecture SQLite DB (`backend/main.py` `_db(lecture_id)` routing to `get_lecture_db_path()`; `flashcard_ratings` + `quiz_attempts` tables) — `FlashcardsPanel.tsx` and `useQuizStore.ts` read and update them via the API rather than in-memory state that resets on mode switch. This delivers the retention loop NorAI's roadmap flags (README/PROJECT_PROGRESS mention SRS — now a true SM-2 scheduler, which exceeds NotebookLM's no-SRS review loop) and matches what users call out as the useful part of NotebookLM's study tools ([notebooklm-to-pdf](https://notebooklm-to-pdf.com/blog/notebooklm-flashcards-quiz)). Open delta vs NotebookLM: per-card review history and quiz analytics dashboards.
...
**D. Mind Map / concept map per chapter (≡ NotebookLM Mind Map)** — *Medium, the highest-leverage visual feature.* **✅ shipped** — the Phase D concept map is live in the workspace (floatable cards, node→scoped-question), and Part 6 exports it to static SVG + interactive HTML. Click-to-seek on nodes was folded into the P6.3 video player (chapter-level seek chips on notes/references); exact-moment chunk seeks remain open.
...
2. **Flashcard/quiz session persistence + "review missed only"** (beyond NotebookLM: a true SM-2 scheduler) — **✅ shipped** (flashcard ratings + SM-2 schedule + Missed deck filter + Anki `.apkg` export; quiz attempt history + deterministic missed-question review). Per-card history/analytics dashboards remain open. (B)
