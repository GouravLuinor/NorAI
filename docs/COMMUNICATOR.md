...
- **Status**: Completed
- **Files Created / Modified**:
  - `RENDER_STAGING_PREP.md` — All checklist items (Phases A through H) completed and marked off.
  - `seed_data/` — Executed `PRAGMA wal_checkpoint(TRUNCATE)` on SQLite checkpointers, purged `-wal`/`-shm` sidecars, and force-staged all 435 demo workspace files into Git.
  - `backend/orchestrator.py`, `tutor/retriever.py`, `tutor/build_index.py`, `tutor/build_screenshot_index.py` — Standardized Chroma client instantiation through `get_persistent_client` with `anonymized_telemetry=False`.
  - `.dockerignore` — Added `.tmp/` and `chroma_db/` to prevent Docker context bloat.
  - `backend/requirements.txt` — Removed stale duplicate file.
- **Verification**:
  - `scripts/run-tests.sh`: 39/39 offline test suites passed (0 failed).
  - `npm run lint && npm run build`: 0 lint errors, built in 593ms.
- **Hand-off Notes / Next Steps**: Ready for Git commit + push, followed by Render web service setup.

### [2026-08-16] — Antigravity: Seeded 3 Demo Workspaces, Docker Startup Sync & Frontend Showcase
- **Agent**: Antigravity (IDE)
- **Status**: Completed
- **Files Created / Modified**:
  - `seed_data/` — Bundled full artifacts for all 3 demo lectures (Neural Networks `ab648382-638f-4dde-b7c1-4007a2e638bb`, Economics `e54d7376-0e7b-472a-9ca6-9b21ad0b2710`, and Open-Weights AI `506dd685-05f9-43df-8d09-5b944c7392f5`). Total size: 36 MB.
  - `backend/main.py` — Added `DEMO_LECTURES` and `DEMO_LECTURE_IDS`, implemented `sync_demo_seed_data()` in FastAPI `lifespan` for zero-configuration container hydration on Render/VPS, granted universal public access in `ensure_lecture_access()`, and prepended demos with `is_demo: True` in `GET /lectures`.
  - `frontend/src/pages/LandingPage.tsx` — Added an "Explore 3 Live Demo Workspaces" section with 1-click launch cards.
  - `frontend/src/pages/UploadPage.tsx` — Added demo badges to the recent lectures list.
  - `DEPLOYMENT_PLAN.md` — Updated Section 1 with the exact 3 demo lecture IDs and features.
- **Verification**:
  - `npm run lint && npm run build` (0 errors, built in 870ms).
  - API smoke test on all 3 demo workspaces (`/outline`, `/notes/1`, `/assessment/1`, `/flashcards/1`, `/chat` — all passed).
- **Hand-off Notes / Next Steps**: Ready for Render cloud staging deployment or Hostinger VPS rollout.

### [2026-08-16] — Antigravity: YouTube Ingestion 403 Forbidden Root-Cause Fix & Multi-Tier Fallback
- **Agent**: Antigravity (IDE)
- **Status**: Completed
- **Files Created / Modified**:
  - `ingest/ingest.py` — Fixed YouTube download `HTTP Error 403: Forbidden`. Removed the `ios` client from yt-dlp `player_client` (which triggers YouTube's GVS PO token requirement and 403 blocks), added URL sanitization (`sanitize_youtube_url`), implemented a 4-tier client & format fallback download loop (`android_vr` → `android` → `mweb` → `generic web`), added non-mp4 auto-remuxing to guarantee `.mp4` for the visual pipeline, and upgraded `probe_video_metadata` with the new clients for instant 2s pre-flight estimation.
- **Verification**:
  - `python backend/test_upload_validation.py` (20/20 passed).
  - End-to-end download & audio extraction verified on previously failing video `https://youtu.be/zxVoCw3P1Gc`.
  - Backend `/estimate` endpoint returned valid metadata (`duration_min: 19.18`) in 2 seconds.
- **Hand-off Notes / Next Steps**: YouTube downloads are now resilient against client throttling and 403 Forbidden blocks.

### [2026-08-16] — Antigravity: Production Deployment Plan, Demo Workspaces & Razorpay Integration
- **Agent**: Antigravity (IDE)
- **Status**: Completed
- **Files Created / Modified**:
  - `DEPLOYMENT_PLAN.md` — Updated end-to-end production deployment guide covering Hostinger KVM 1 VPS, Caddy reverse proxy, Cloudflare SSL/DNS, single-container Docker setup, 3 demo lecture recommendations (STEM, CS/AI, Biology/Economics), 15-min free processing limit, and Razorpay payment roadmap.
  - `.env.example` — Added `NORAI_ENV`, Razorpay keys, and production configuration placeholders.
- **Verification**:
  - Validated markdown formatting and environment variables alignment.
- **Hand-off Notes / Next Steps**: Ready for user to process the 3 new demo lectures and proceed with VPS deployment.

### [2026-08-16] — Antigravity: NorAI Marketing Brochure & PDF Generation
- **Agent**: Antigravity (IDE)
- **Status**: Completed
- **Files Created / Modified**:
  - `PRODUCT_BROCHURE.md` — Non-technical, high-converting EdTech SaaS marketing brochure.
  - `docs/brochure.html` — Responsive, dark-mode SaaS presentation and print-ready styled brochure.
  - `PRODUCT_BROCHURE.pdf` & `docs/PRODUCT_BROCHURE.pdf` — High-resolution, full-color PDF brochure generated via headless Chromium with exact A4 print pagination.
  - `scripts/generate_pdf.py` — Automated PDF conversion script using Playwright.
- **Verification**:
  - `PRODUCT_BROCHURE.pdf` generated successfully (1.36 MB) with background graphics, typography, badges, and layout intact.
- **Hand-off Notes / Next Steps**: Ready for sharing with users, marketing, and distribution.

### [2026-08-15] — Antigravity: Real-Time Reactive Thread Title Renaming & Thread 1 Retention Fix
- **Agent**: Antigravity (IDE)
- **Status**: Completed
- **Files Created / Modified**:
  - `frontend/src/stores/useThreadStore.ts` — Added reactive `labels: Record<string, string>` store state, `setThreadLabel` action, and updated `loadThreads` to merge backend and local threads preserving `'default'` and uncommitted threads without dropping existing threads on reload.
  - `frontend/src/lib/threadStorage.ts` — Exported `getLabels` and `saveLabels` for reactive store sync.
  - `frontend/src/lib/chatApi.ts` — Updated `ensureLabel` to synchronously invoke `useThreadStore.getState().setThreadLabel(threadId, title)` on the first message sent.
  - `frontend/src/components/layout/Sidebar.tsx` — Subscribed to reactive `labels` state from `useThreadStore` and rendered `labels[t] || getThreadLabel(t)` so thread titles update live instantly in the UI.
  - `backend/main.py` — Updated `GET /threads` endpoint to respect `deleted_threads` and guarantee `"default"` is returned unless explicitly deleted.
- **Verification**:
  - `scripts/run-tests.sh` (39/39 backend test suites PASSED).
  - `cd frontend && npm run lint && npm run test` (0 lint warnings/errors, 73/73 Vitest unit tests PASSED).
  - `cd frontend && npm run build` (Clean production bundle built in 629ms).
- **Hand-off Notes / Next Steps**: Both thread issues reported by the user are resolved. Threads now update titles instantaneously upon asking the first question, and Thread 1 / default threads are preserved across reloads.

### [2026-08-15] — Antigravity: Production Roadmap & Deployment Plan Sync (Gemini API Transcription)
- **Agent**: Antigravity (IDE)
- **Status**: Completed
- **Files Created / Modified**:
  - `ROADMAP.md` — Updated item P1.7, P5.4, and P1 Definition of Done to reflect cloud-based Google AI Studio Gemini API transcription (`gemini-3.1-flash-lite`), 18-minute parallel audio chunking, 9.2× speedup (53s vs 498s CPU for 71m lecture), 0% host CPU load, and usage ledger integration under `"transcription"` stage.
  - `DEPLOYMENT_PLAN.md` — Updated Executive Summary, Architecture diagrams, §0 VPS fit assessment, §5 `.env.production` template, and §7 Checkpoints/scaling triggers to highlight that the Hostinger KVM 1 (1 vCPU / 4 GB) is completely relieved of CPU transcription constraints.
  - `PROJECT_PROGRESS.md` — Updated Phase P1 summary notes to reflect cloud transcription architecture.
- **Verification**:
  - `scripts/run-tests.sh` (39/39 test suites PASSED, including `transcription/test_gemini_transcription.py` and `transcription/test_transcribe_config.py`).
  - `cd frontend && npm run lint && npm run build` (0 warnings, 0 errors, clean build in 693ms).
  - `cd frontend && npm run test` (73/73 Vitest unit tests PASSED).
- **Hand-off Notes / Next Steps**: Production documentation and roadmap are 100% aligned with cloud transcription. Ready for pre-deploy smoke testing and launch.
...
- **Agent**: OpenCode (CLI)
- **Status**: Completed — all gates green (backend 42/42 suites · oxlint clean · vitest 73/73 · vite build OK).
- **Context**: Phase 2 of AUDIT_2026-08-22.md §5. Committed in ee7d914 (with Phase 3).
- **Done**:
...
### [2026-08-22] — OpenCode: Phase 4 "UI/UX Tier-0 Polish" Complete
- **Agent**: OpenCode (CLI)
- **Status**: Completed — gates green (oxlint clean · vitest 73/73 incl. updated UsagePage error-copy test · vite build OK). Committed in 26c521a.
- **Done (4.1 states)**: Button base bakes `disabled:opacity-60`; outline/surface variants gain active press. SegmentedControl: Arrow/Home/End roving selection + focus, `role="group"`, FOCUS_RING + press on items; AssessmentView questions/history toggle + difficulty filter now use it. Quiz MCQ/TF answer buttons: FOCUS_RING + `active:scale-[0.98]` + disabled styling; letter chips `text-npfg`. Upload recent-lecture cards: ring + press. Landing nav links get LINK_RING; header CTAs/mobile menu button ringed; secondary hero CTA gains press. InputZone send disabled when empty (+ half-step/ripple classes removed). Select: Home/End, 500ms typeahead, Escape refocuses trigger, new `disabled` prop.
- **Done (4.2 tokens)**: raw reds → nr/nrb/nrbr in AppErrorBoundary/PrintErrorBoundary/PrintPage/UploadPage(×2)/ShareModal revoke btn; ConceptMap root/selected node blur shadows → shadow-ev2/ev1 hairline treatment; radius zoo ([5px]/[3px]/[1px]) → rounded-sm/xs everywhere; Material ripple CSS deleted (index.css) — zero `.ripple` usages remain; motion tokens added (`--dur-fast` 150ms / `--dur-panel` 240ms / `--ease-out-expo`); quota fills duration-300→200, ink rail 500→300; half-step spacing snapped (Sidebar p-3.5→p-4, w-5.5→w-6, AIPanel px-3.5→px-4, QuizPanel p-3.5/gap-2.5/gap-3.5); tabular-nums added to Sidebar quota, Flashcards position, SearchBar counter, AssessmentView count, ProcessingPage %, ReferencesPanel badge, Upload estimates/chapter counts.
- **Done (4.3 loading/error)**: NEW lib/errorCopy.ts friendlyError() mapper applied at ChatArea assistant-bubble, AuthModal password+google errors, UsagePage, BillingPage, ProcessingPage stage-error copy. Sidebar chapters: skeleton rows while chaptersLoading (new store flag) + honest empty copy; threads empty state. CoursesPage skeleton while coursesLoading (new store flag) — no more false "No courses yet" flash. FlashcardsPanel bare one-liner → EmptyState card with guidance. Usage/Billing loading text → SkeletonCard stacks. ProcessingPage errored state gets Retry (reload) + "start a new upload" CTA. Toasts: errors auto-dismiss 8s vs 3s info/success. AIPanel quiz fetch: success toast only when questions load; empty → info toast; failure → friendlyError toast (no more silent catch). AssessmentView fetch failures surface via EmptyState+Retry (loadError state); Start-Quiz with 0 matches now toasts why. QuizPanel evaluating state hides prior controls and shows spinner status.
- **Done (4.4 a11y)**: ConceptMap nodes are real `<button>`s (aria-pressed, aria-label per concept, FOCUS_RING); zoom in/out/reset + floating-card close icon buttons labeled; ShareModal tutor-chat switch gets aria-labelledby + focus ring + token knob (bg-white removed); BillingPage anchor>button nesting fixed — anchors styled as buttons at all 3 sites; Workspace mobile-sidebar + tablet-AI drawers mounted through house Dialog primitive (focus trap, Escape, aria-modal, focus restore); Dialog overlay alignment now overridable (alignment moved from hardcoded base into default overlayClassName — API-compatible); ShortcutsModal phantom Ctrl+F entry removed; Landing demo-mock headings demoted to <p> so document order is h1→h2 clean.
- **Done (4.5 primitives)**: NEW ui/EmptyState.tsx (icon chip + title + hint + action). NEW ui/NumberTicker.tsx (textContent-direct count-up, expo-out, reduced-motion instant) driving the four Usage summary cards.
- **Not done (explicitly deferred)**: [P] unified Tabs primitive (SegmentedControl keyboard parity shipped instead); [P] Button size/density props; [m] CitationBox hover-only popover + HighlightAsk mouseup-only selection (keyboard selection paths); [P] Sidebar delete-confirm symmetry. Audit acceptance axe-pass/keyboard-walkthrough/visual-diff need dev servers + browser MCP — manual follow-up recommended before Phase 5 merge.
- **Pending / Next**: Phase 5 (architecture & simplification — largest blast radius, do with CI green and no pipeline work in flight). Phase 4 changes uncommitted.

---

### [2026-08-22] — OpenCode: Phase 5 "Architecture & Simplification" Complete
- **Agent**: OpenCode (CLI) + 5 parallel opencode subagent workers (deepseek-v4-flash via `opencode run`; note: `--attach` to `opencode serve` is broken in CLI 1.18.21 — run detached instead; paid Zen models fail without a payment method, free inherited model works).
- **Status**: Completed — gates green: suite **42/42**, oxlint clean, vitest **73/73**, vite build OK. Uncommitted.
- **Done (5.3a deletions)**: generate_pdfs.py deleted; unused init_db() removed (db/database.py + __init__ export); dup imports consolidated (main.py, extract/extractor.py); dead chapter_summary_node removed from tutor/nodes.py.
- **Done (5.1 router split)**: backend/main.py **3175 → ~860 lines**. Six domain routers under backend/routers/ (`webhooks`, `billing`, `courses`, `quiz`, `content`, `tutor`), mounted prefix-free so all paths unchanged. Shared helpers extracted to **backend/access.py** (ensure_lecture_access, _assert_shared_or_404, persona hardening, guest/demo caps, usage flushers) and **backend/lecture_db.py** (_db + all DDL-ensure helpers). Concept-map builder is now a pure function in **backend/concept_map.py**. Chat/threads SSE endpoints live in routers/tutor.py and call LLMs through `tutor_deps.<fn>` indirection — tests must patch **backend.dependencies** (not main_mod) for ainvoke_tutor/astream_tutor_tokens.
- **Done (5.2 gemini factory)**: single client + retry/RPM policy in **backend/gemini_client.py**; rewired tutor/llm, tutor/embedding, tutor/cache, extract/extractor, transcription/gemini_transcriber (+notes/visual sites). make_chat_llm seam preserved (offline tests unaffected). Remaining direct `genai.Client(` sites are docstring examples only.
- **Done (5.3b/c)**: output_dir now required across extract/chunking/transcription/notes(+outline)/visual/flashcards/assessment/revision_notes (acceptance grep = 0 hits; orchestrator already explicit). Frontend dedupe: shared useAskNora hook, lib/id.ts, consolidated stripSources (ChatArea/ConceptMapView/HighlightAsk/useThreadStore).
- **Done (5.4a quiz typing)**: Pydantic Question/AnsweredQuestion models on /quiz payloads (extra fields ignored, required enforced; attempts JSON stored via model_dump). test_phase1_hardening fixture updated to send a valid question object.
- **Worker reports**: .tmp/opencode/reports/{content-router,output-dir,frontend-dedupe,tutor-router,gemini-client}.md
- **Pending / Next**: Phase 6 durability & ops (per docs/ROADMAP.md). Phase 5 changes uncommitted — commit on request.
