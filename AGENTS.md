...
`README.md` (architecture + pipeline), `PROJECT_PROGRESS.md` (living current-state doc — update when making notable pipeline/backend/frontend changes), `COMMUNICATOR.md` (shared agent handoff bridge — check and update after completing tasks), `NOTES.md` (developer notes: fix log, pricing/decisions, open items), `fix.md` (severity-ranked chief-reviewer findings + applied fixes from the 2026-08-15 dynamic-scaling audit), `DEPLOYMENT_PLAN.md` (agreed production deployment path — Hostinger VPS, single-container, Caddy, Supabase), `UI_UX_AUDIT_REPORT.md` (frontend audit — fully executed), `NotebookLM_competitive_analysis.md` (forward feature ideas), `audit/audit_tutor_and_auxiliary.md` (tutor/RAG/frontend audit detail), `ROADMAP.md` (phased engineering-hardening plan — security → economics → billing → RAG → durability → foundation → retention; update item status as work lands), `docs/token_reduction.md` (P7 token-reduction sprint: cost baseline, prompt compression, context caching, output-token cuts).
