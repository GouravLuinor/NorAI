...
- `backend/` — FastAPI app. `main.py` (app factory, lifespan, middleware, static/SPA catch-all; ~850 lines) + `backend/routers/` domain routers (`webhooks`, `billing`, `courses`, `quiz`, `content`, `tutor`) mounted without prefixes so paths are unchanged. Shared helpers live in `backend/access.py` (auth/access checks, guest caps, persona handling, usage flushers) and `backend/lecture_db.py (`_db` + DDL-ensure helpers). `orchestrator.py` runs the full 18-stage pipeline synchronously inside a `backend/jobs.py` worker thread; progress is persisted to the Lecture DB row and polled by the frontend — no SSE. `backend/gemini_client.py` is the single Gemini client factory + retry/RPM policy used by tutor/, extract/, transcription/, notes/, visual/.
...
- **Handoff:** keep AGENTS.md authoritative for both; write a short handoff-log entry (done / pending / commands to verify) in `docs/COMMUNICATOR.md` at each switch so the other side can resume without re-explaining.
...
`README.md` (architecture + pipeline), `docs/PROJECT_PROGRESS.md` (living current-state doc — update when making notable pipeline/backend/frontend changes), `docs/COMMUNICATOR.md` (shared agent handoff bridge — check and update after completing tasks), `docs/NOTES.md` (developer notes: fix log, pricing/decisions, open items), `docs/fix.md` (severity-ranked chief-reviewer findings + applied fixes from the 2026-08-15 dynamic-scaling audit), `docs/DEPLOYMENT_PLAN.md` (agreed production deployment path — Hostinger VPS, single-container, Caddy, Supabase), `docs/RENDER_STAGING_PREP.md` (staging deployment & container verification checklist), `docs/UI_UX_AUDIT_REPORT.md` (frontend audit — fully executed), `docs/NotebookLM_competitive_analysis.md` (forward feature ideas), `docs/ROADMAP.md` (phased engineering-hardening plan — security → economics → billing → RAG → durability → foundation → retention; update item status as work lands), `docs/token_reduction.md` (P7 token-reduction sprint: cost baseline, prompt compression, context caching, output-token cuts).

`docs/TUTORIAL.md` (full-project tutorial + interview prep) — agents do NOT need to read it for working on the codebase; it's large and would fill the context window unnecessarily. Only update it from time to time when significant changes land, to keep it in sync with the project.