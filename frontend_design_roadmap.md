...
Status: Tier 0, 1, 2, 3, 5 complete (Tier 3 fully QA-green: **Lighthouse a11y 100 on both themes** across notes/revision/assessment/quiz, Pass A review clean, Pass B Chrome MCP clean, Pass C visual audit clean). Tier 5 fully QA-green incl. dark-theme AA contrast fix. **Architect's Sketchbook** theme layer implemented (vellum light default + Prussian dark, red-pencil accent, hard-offset shadows, grid/grain/fold-marks, Space Grotesk + Newsreader, ProcessingPage schematic). **Dark-mode revision complete**: "Luminous Blueprint at Night" — lifted Prussian canvas, light-edge shadows, luminous grid, brighter accent/dots, pre-paint theme persistence (Pass C re-audit PASS). **Tier 4 complete** (signature typography pass — minimal scope: `text-hero` token + tiny-size consolidation, quiz-mode switch fix + gated toast, evaluate-error surfacing, partial-content badge; QA-green). Next: none — roadmap complete.
...
- [x] 10. Build `src/components/ui/` primitives: `Button`, `IconButton`, `Card`, `Input`, `SegmentedControl`, `Badge`. Migrate the ~15 copy-paste sites (primary button, ghost button, card container, section-label). **Blueprint grammar:** hairline ink border + `--shadow-bp-*` hard shadow + optional fold-mark; icons at lucide `strokeWidth={1.5}`; section labels in mono ALL-CAPS. — **Done:** primitives in `src/components/ui/` (`shared.ts` `FOCUS_RING`); migrated NotesView, RevisionView, PrintPage, FlashcardsPanel (rating → SegmentedControl), AIPanel (mode switch), DocPanel, Sidebar, Workspace, QuizPanel, AssessmentView, HighlightAsk, SearchBar (Input + IconButtons + focus-within ring), UploadPage (source tabs → SegmentedControl), Lightbox, ShortcutsModal, ToastContainer. Deliberate holds: QuizPanel options (3-state), assessment True/False print stubs (static), UploadPage lecture rows stay `<button>` (Card is a div — a11y), ChapterScreenshots toggle (ghost hover bg).
- [x] 11. Extract a single shared markdown `components` object (`src/lib/markdown.tsx`) + `headingToId` util (dedupe 4× + 5×). — **Done:** `createMarkdownComponents({chat?, headingIds?})` factory + `docMarkdownComponents` (NotesView), `revisionMarkdownComponents` (RevisionView), `printMarkdownComponents` (PrintPage), `chatMarkdownComponents` (MessageBubble); `headingToId`/`extractText`/`cleanImageSrc` shared; the 3 inline `headingToId` copies (ChatArea, HighlightAsk, `useThreadStore` ref-builder) consolidated into `src/lib/references.ts` `buildReferences` (deduped ChatArea, HighlightAsk, store `loadThreadMessages`).
- [x] 12. Wire real SSE streaming: `sendChatMessageStream` exists but is dead — `ChatArea` consumes it so the loading shimmer is honest. — **Done:** backend `chat_stream` replays the already-computed answer in 24-char word-aligned JSON-wrapped chunks (2ms sleep; never slower than the non-stream path) + `{"final": …}` framing; `ChatArea` consumes the stream (transient bubble from `streamingText`, Sources appendix stripped live, atomic dedup append on `final`, per-call AbortController + abort-on-thread-switch + `streamingText` cleared on switch, error path preserved); `HighlightAsk` keeps the non-stream `/chat` path.
- [ ] 13. Decompose monoliths: split `useThreadStore.ts` (chat API layer + store), `PrintPage.tsx` (error boundary, card classifier, markdown renderers, data fetching).
   - [x] 13a. `useThreadStore.ts` — **Done:** chat API layer → `src/lib/chatApi.ts` (`apiFetch`, `sendChatMessage`, `sendChatMessageStream`), localStorage/labels → `src/lib/threadStorage.ts`; store is now store-only, re-exporting for back-compat.
   - [x] 13b. `PrintPage.tsx` decomposition (error boundary, card classifier/Wrapper, data fetching, content renderer). — **Done:** new `src/components/print/` — `types.ts`, `cardClassifier.ts`, `parseChapter.ts`, `PrintSectionCard.tsx`, `PrintErrorBoundary.tsx`, `usePrintData.ts`, `PrintChapter.tsx`, `PrintAssessmentChapter.tsx`; `PrintPage.tsx` is now a thin entry. Behavior-preserving (title/preamble/sections, `__PRINT_DATA__` → `printDataReady` → 500 ms fallback fetch, 3 s auto-print).
- [x] QA: build + lint green, Pass A review clean, Pass B Chrome MCP clean, Pass C vision audit clean. — **Tier 2 complete** (review: `/.tmp/review-tier-2.md`; screenshots `/.tmp/qa-tier-2/screenshots/`; Pass C visual audit done by human reviewer).

### Tier 3 — Accessibility & Web Interface Guidelines compliance

- [x] 14. Clickable `<div>`/`<li>` → `<button>`/`<Link>` with keyboard path: `Sidebar` (chapters/threads), `assessment-cards` (MCQ, answer key), `ReferencesPanel`, `FlashcardsPanel` (flip), `ChapterScreenshots` (zoom).
- [x] 15. `:focus-visible` double-ring pattern app-wide; replace `outline-none`-only inputs.
- [x] 16. Modals → `role="dialog"` + `aria-modal` + focus trap + focus restore (`ShortcutsModal`, `Lightbox`); fix Escape collision.
- [x] 17. `aria-live` on ToastContainer + chat streaming; `aria-label` on all icon buttons (`SearchBar`, `ShortcutsModal`).
- [x] 18. Contrast fixes: bump `nt3`/`nt4`, ensure primary-button text ≥ 4.5:1 AA (red-pencil fill uses ink or white per theme — verify in Pass C).
- [x] 19. Keyboard-resizable panels (`role="separator"`, `aria-valuenow`, arrow keys); `Intl.RelativeTimeFormat`; `…` not `...`; image `width`/`height`.
- [x] QA: build + lint green, Pass A review clean, Pass B Chrome MCP clean, Pass C vision audit clean (incl. Lighthouse a11y). — **Tier 3 complete** (review: `.tmp/qa-tier3/passA-notes.md`; browser test: `.tmp/qa-tier3/passB-notes.md`; visual audit: `.tmp/qa-tier3/visual-review.md`; screenshots `/.tmp/qa-tier3/screenshots/`; Lighthouse a11y 100 light+dark on Study notes/Revision/Assessment/Quiz).

### Tier 4 — Signature typography pass

- [x] 20. Typography ramp: collapse tiny sizes into the real `--text-*` scale; **Space Grotesk** (chrome/hero/pipeline/panel headers) + **Newsreader** (reading pane titles/H1s) + mono metadata; tight tracking on display. — **Minimal scope (user-approved): added `--text-hero` (26px) for doc H1s, `text-[26px]` → `text-hero` in NotesView/RevisionView, remaining `text-[9px]` spec-prefixes → `text-3xs` (DocPanel, UploadPage). 21px print titles + 15px quiz questions left as deliberate reading/display sizes.**
- [x] 21. Signature element: refined "Nora" wordmark in **Space Grotesk** in the AI panel + live status dot. — **Verified already consistent (AIPanel header: Space Grotesk, wordmark + `01 · TUTOR` + live status dot); no change needed.**
- [x] 22. Consistent empty/loading/error states; fix quiz `finishQuiz` silent-failure (`QuizPanel.tsx:189-191`) and unconditional "Quiz started" toast (`AIPanel.tsx:59`); add `incomplete: true` warning badge (from tier2 plan Open Question 2). — **QuizPanel now surfaces evaluate failure with a Retry button (`role="status"` error box, tokens match feedback box); AIPanel switches to quiz mode eagerly (so an empty/errored fetch lands on the "No quiz questions loaded" state) and toasts only when questions actually load; new `PartialContentBadge` (subtle red-tint chip, `role="status"`) in NotesView + RevisionView when the backend's graceful-degradation marker ("partially degraded") is present. Assessment excluded deliberately — `/quiz/questions` returns `[]` on both "degraded" and "not generated", so it cannot be distinguished frontend-only.**
- [x] QA: build + lint green, Pass A review clean, Pass B Chrome MCP clean, Pass C vision audit clean. — **Tier 4 complete.** Build ✓ lint ✓ (3 pre-existing exhaustive-deps only). Pass A ✓ (web-design-guidelines: badge/error use `role="status"`, retry button `FOCUS_RING`, lucide `aria-hidden`, error copy has next step). Pass B ✓ (Chrome MCP — H1 computed 26px Newsreader via `text-hero`, spec labels 9px via `text-3xs`, quiz mode now switches correctly, no console errors, both themes, Lighthouse a11y 100 / best-practices 100; SEO/agentic dips are snapshot-mode SPA artifacts). Pass C — screenshots `.tmp/qa-tier4/screenshots/`.
...
> **Every tier, without exception, Pass A loads the `frontend-design` and Web Interface Guidelines review skills and applies them to that tier's diff (see "Skills required — MANDATORY" below). A tier is only "complete" once its Pass A review is clean; a review that skipped either skill is invalid and must be re-run.**
...
- Skills required — **MANDATORY, gate on Pass A**. The reviewer MUST load BOTH skills before reviewing and list which it loaded (and the fetched-guidelines date) at the top of the review; loading is verified against the tier's diff, so a review that skipped a skill is INVALID and must be re-run:
  1. **`frontend-design`** skill — audit any new UI against its guidance (distinctive, non-templated signature; restraint — boldness spent in one place; type/palette derived from the DESIGN.md system; copy written from the user's side; quality floor of keyboard focus / reduced-motion / responsive).
  2. **Web Interface Guidelines** skill (`web-design-guidelines` in `~/.agents/skills/web-design-guidelines`, a.k.a. "vercel_guidelines") — the reviewer MUST `WebFetch` fresh rules from `https://raw.githubusercontent.com/vercel-labs/web-interface-guidelines/main/command.md` and check the tier's UI against them (a11y, focus states, forms/labels, animation, typography, images, performance, dark mode, etc.), reporting findings as `file:line`.
- Scope: only the files changed during the tier (diff via `git diff` against pre-tier state).
- Checks: correctness, the tier's specific checklist items, regressions, dead code introduced, `any` usage, `console.log` leftovers, DESIGN.md / web-interface-guidelines compliance for new UI (both skills above). For any new UI, also audit against the `frontend-design` guidance (same list as the skill above — signature, restraint, DESIGN.md type/palette, user-side copy, quality floor).
...
- [x] Streaming: decide whether real SSE replaces non-streaming path or exists as fallback (Tier 2.12). — **Decided: `/chat/stream` is the primary ChatArea path, replay-after-ready with chunked (not per-char) frames so it is never slower than non-streaming. The non-stream `/chat` endpoint stays as the fallback/other entry point (HighlightAsk) and for any client without SSE support. Implemented in Tier 2 item 12.**
...
- **Tier 2**: `[x]` **complete** — items 11 ✓ (shared markdown module + `headingToId`, 4 renderer sites deduped), 12 ✓ (SSE streaming: backend chunked `/chat/stream` + ChatArea live bubble), 13a ✓ (`useThreadStore` decomposed → `lib/chatApi.ts` + `lib/threadStorage.ts`), **10** ✓ (ui primitives `Button`/`IconButton`/`Card`/`CardHeader`/`Input`/`SegmentedControl`/`Badge` + `FOCUS_RING`; full-sweep migration across NotesView, RevisionView, PrintPage, FlashcardsPanel, AIPanel, DocPanel, Sidebar, Workspace, QuizPanel, AssessmentView, HighlightAsk, SearchBar, UploadPage, Lightbox, ShortcutsModal, ToastContainer), **13b** ✓ (PrintPage decomposed → `components/print/*`, thin entry `PrintPage.tsx`). Build ✓ lint ✓ (no new warnings — 3 pre-existing exhaustive-deps). QA ✓: Pass A (skill-gated code review, 2 a11y fixes), Pass B (Chrome MCP, 1 a11y fix — URL input `id`/`name`), Pass C (human visual audit of `/.tmp/qa-tier-2/screenshots/`). Review: `/.tmp/review-tier-2.md`.
- **Tier 3**: `[x]` **complete** — a11y & Web Guidelines pass. Items 14 ✓ (clickable divs/li → real `<button>`s with keyboard paths: Sidebar chapters/threads, QuizPanel MCQ/answer-key toggles, ReferencesPanel rows, FlashcardsPanel flip, ChapterScreenshots zoom + "Important Visuals" toggle with `aria-expanded` + `FOCUS_RING`), 15 ✓ (FOCUS_RING double-ring on all interactive primitives + quiz options + chapter zoom buttons), 16 ✓ (ShortcutsModal + Lightbox are real `role="dialog"`/`aria-modal` modals with focus trap + restore + scroll lock; Escape collision with the collapse-Escape guard fixed; Lightbox img width/height + caption via `aria-describedby`), 17 ✓ (ToastContainer `aria-live="polite"`; chat streaming live-region; `aria-label` on SearchBar + ShortcutsModal icon buttons; Sidebar `<select aria-label="Select lecture">`), 18 ✓ (contrast: light `nt3 #72675A→#6A5F53`/`nt4 #786C60→#6D6155`, dark `nt4 #7A93B0→#A3BBD6`; accent-text token system `-t` light/dark per accent + dark fill tokens `ngfill/nrfill` for white-on-green/red quiz chips; sidebar chapter numbers `opacity-70`→`text-nt4`, active chapter ink on red tint; `.note-callout` label → `--color-npt`; UploadPage sr-only `<h1>`; AI-panel `h3`→`h2`; doc root `<main>`), 19 ✓ (resize separators `role="separator"` + `aria-valuenow` + ArrowLeft/Right 16px + Home/End, verified with real Chrome `press_key`). Build ✓ lint ✓ (only 3 pre-existing exhaustive-deps). QA ✓: Pass A (code review, 0 critical/4 major/27 minor-nit → majors fixed: Dialog `onClose` ref-stabilized effect, ReferencesPanel `inert={collapsed}`, quiz dark chips, callout label), Pass B (Chrome MCP + Lighthouse — a11y 100 both themes on notes/revision/assessment/quiz; keyboard passes; stale console issue verified DOM-clean), Pass C (MiMo vision audit, 0 critical/major/3 minor/4 nits; minor #2 badge contrast covered by accent-text tokens). Review: `.tmp/qa-tier3/passA-notes.md`, `.tmp/qa-tier3/passB-notes.md`, `.tmp/qa-tier3/visual-review.md`.
- **Tier 4**: `[ ]` not started
- **Tier 5**: `[x]` **complete** — Architect's Sketchbook theme layer (reboot): fonts loaded (Space Grotesk + Newsreader, Google Fonts `display=swap` + preconnect); token re-skin with **polarity flip** (vellum `#F6F2E7` light default + Prussian `#0A1628` dark override, red-pencil `#C2410C`/`#D1552E`/`#E85D2F`, status dots pinned, hairline ink borders); hard-offset `--shadow-bp` (`3px 3px 0 0`) with `ev2/ev3` re-pointed (54 call sites cascade, zero edits); `.bg-blueprint-grid`/`.noise`/`.fold-marks`/`.spec-label`/`.note-callout`/`.dimension-marker` utilities; spec numbering `01.`–`NN.` (pipeline stages, sidebar sections, panel headers); ProcessingPage rebuilt as schematic showpiece (numbered spec rows + self-drawing ink rail via framer-motion, dashed→solid progress line, `useReducedMotion`-gated); UploadPage hero leader-line diagram + `NOTE:` callouts in NotesView; lucide `strokeWidth={1.5}` hairline sweep (71 icons); 8 viridian gradients removed (flat `bg-np`), `shadow-np/30` glow → `shadow-bp`; press-down CTAs (`active:translate-y-[1px]` + shadow collapse); `ThemeToggle` default flips to `light`, meta `theme-color` → `#F6F2E7`/`#0A1628`; **dark-theme AA contrast fix** (Pass A/C finding — user-approved): added `--color-npf`/`--color-npfh` fill tokens (light `#C2410C`/`#A9361C`, dark `#C43D1E`/`#B93A1D` — white-on ≥5.2:1, AA pass) applied to 22 text-bearing `bg-np` fill sites. **Dark-mode redesign follow-up ("Luminous Blueprint at Night", user-driven audit)** — dark no longer a flat inverted ramp: canvas lifted to luminous Prussian `#0B1E3A` + wider surface ramp (`#112948`→`#274C74`, separation 1.14→1.88:1); **shadow polarity flipped to LIGHT edges** (`--shadow-bp` → `rgba(150,200,255,0.10)` lit cut, was invisible black-on-black); `--color-grid` token added (dark luminous cyan `rgba(140,200,255,0.08)`, light ink 5% unchanged); accent brightened (`np #E85D2F`, `nph #FF6B38`, `npf #C84926`, tint 0.14→0.20); status blue/purple brightened (`#2F7FE0`/`#9A5CC0`); scrollbar/scrollpulse tokenized off hardcoded rgba; **pre-paint theme-init script in `index.html`** (dark persisted across fresh loads of non-workspace routes — was dropping to light); `theme-color` meta → `#0B1E3A`. Build ✓ lint ✓. **Primary Tier 5 QA pass** — Pass A code review in `.tmp/qa-tier5/passA-notes.md`; Pass B Chrome MCP in `.tmp/qa-tier5/passB-notes.md` (23 screenshots in `.tmp/qa-tier5/screenshots/`); Pass C vision audit in `.tmp/qa-tier5/visual-review.md` PASS + `visual-review-fix.md` PASS. **Dark re-audit** — Pass C re-audit ✓ (`.tmp/qa-darkmode/visual-review.md` PASS + `visual-review-fix.md` PASS + `visual-review-light.md` light-regression PASS; 10 screenshots in `.tmp/qa-darkmode/screenshots/`).
