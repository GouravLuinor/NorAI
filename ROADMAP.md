...
| P5.8 | Frontend tests + API contract tests | ✅ | `vitest.config.ts`, `src/test/setup.ts`, 38 Vitest/RTL tests, `backend/test_api_contract_offline.py` [NEW]; `test` script added | Frontend now has a real test stack (repo CI already ran `npm run test` but no script existed — it was failing): Vitest 4 + React Testing Library + jest-dom + jsdom, `test` script (`vitest run`), tests typechecked by the same `tsc -b` (explicit vitest imports, no globals). Unit suites: `parseChapter` (all 5 payload shapes), `references` (confidence gate, verified-citation precedence, rawText fallback only-when-empty), `http` (ApiError detail extraction, non-JSON rejection, abort pass-through, apiFetch null-on-failure), `conceptMapLayout` (determinism, bounds), plus components: `Markdown` (GFM table + KaTeX), `AppErrorBoundary` (recovery panel on render error), `useQuizStore` question_id normalization. Backend: new `backend/test_api_contract_offline.py` boots the app in-process via TestClient with lifespan startup (Alembic migrations on temp SQLite, supervisor daemon) and adds 15 checks — /docs, /openapi exposes /process + /chat + /quota, /lectures + /outline + /quiz/questions shapes, anonymous /quota trial shape, /process + /billing 401 unauthenticated, /process validation negatives (bad source_type, bad youtube/gdrive URLs, upload w/o file), /chat 422. No paid pipeline / no network. `run-tests.sh` picks it up automatically (30/30). Live-server probe `backend/test_api_contract.py` stays excluded. |
...
| # | Item | Status | Ref | Notes |
|---|---|---|---|---|
| P6.1 | Real token streaming | ✅ | `backend/dependencies.py:300` (`astream_tutor_tokens`), `backend/main.py:328` (`/chat/stream`) | `/chat/stream` streams the generate_answer_node LLM tokens via LangGraph `astream_events(version="v2")` (filtered on `langgraph_node`) instead of fake 24-char re-chunks. Runs under the same per-lecture lock + dedupe/zombie guards as `ainvoke_tutor`; `tutor/llm.py` `UsageLoggingChatLLM.astream` logs streamed usage. Wire contract unchanged. **Live-verified 2026-08-12 against real Gemini** on a grounded lecture: 11 incremental token frames (TTFT 9.74s incl. retrieval + rewrite) instead of one whole-answer frame. Two fixes the live test caught: `generate_answer_node` must use `llm.astream` (was `ainvoke` → 1 huge chunk), and the node filter must match the registered node name `generate_answer` (was `generate_answer_node` → every event dropped, whole answer re-emitted via fallback). Covered by `tutor/test_tutor_streaming.py` (5 checks) + `tutor/test_async_persistence.py`. |
| P6.2 | Spaced repetition (SM-2) + Anki export | ✅ | `flashcards/sm2.py`, `backend/main.py:240,900,1067` (`/flashcards/ratings`, `/flashcards/export`), `frontend/src/lib/flashcardSchedule.ts`, `frontend/src/components/flashcards/FlashcardsPanel.tsx` | `flashcards/sm2.py` `apply_sm2` persists ease/reps/interval/due per card (lazy `easiness`/`reps`/`interval_days`/`due_at`/`last_reviewed_at` columns on `flashcard_ratings`); `POST /flashcards/ratings` upserts + returns the new schedule; `GET /flashcards/export` builds a real Anki `.apkg` (genanki) for download. Frontend `FlashcardsPanel` renders the deck with filters (All / Missed = Again+Hard / Due = scheduled-or-new), SM-2 due badges, footer stats scoped to the deck, and an Export Anki button. **QA-passed 2026-08-12** (live browser, real cards): ratings persist, schedule advances (Good → IN 1D, deck leaves Due), `.apkg` downloads, empty states render; two bugs fixed (filtered-deck badge read the wrong card's schedule; rating the last card of a shrinking deck rendered a blank card). Covered by `frontend/src/lib/flashcardSchedule.test.ts`. |
| P6.3 | Click-to-video grounding | ✅ | `backend/video_map.py`, `backend/main.py:1509` (`/video-map`), `frontend/src/stores/useVideoStore.ts`, `frontend/src/components/video/VideoPlayer.tsx` | Collapsible YouTube player docked in the DocPanel header streams straight from YouTube (local copy stays transient — `orchestrator.py` `_TEMP_DIRS` deletes it post-run). `/lectures/{id}` now merges DB `source_type`/`source_url`/`status`/`duration_seconds`; `/video-map` returns per-chapter + per-chunk timestamps from `merged_objects/chunk_*.json`. Seeks flow through the store (buffered until the player is ready) from sidebar chapter rows, chapter headers in NotesView, and tutor note-references (mapped to chapters via `ch{chapter_id}__` chunk-id prefix — no backend citation enrichment). Non-YouTube sources + non-completed statuses degrade to a hidden/placeholder bar. Pipeline fix: `KnowledgeObject` now persists chunk `start`/`end` (were dropped → no-visual merged objects always read 0). **Live-run catch 2026-08-13** (13.6-min lecture): LLM-produced visual objects also dropped timestamps (`VisualObjectItem` lacked `start`/`end`, so only fallback objects carried them → only 4/24 chunks seekable); added the fields and populate them from the chunk mapping, and `merge_objects` now falls back to knowledge times when visual times are absent (also enables zero-LLM backfill of merged objects). Covered by `backend/test_video_map.py`, `frontend/src/lib/video.test.ts`, `frontend/src/stores/useVideoStore.test.ts`, `frontend/src/components/video/VideoPlayer.test.tsx`. |
...
- **P6**: streaming is token-level (✅ P6.1); a flashcard deck exports to Anki (✅ P6.2); every citation can seek the video (✅ P6.3 — chapter-level best-effort seek, DoD closed; exact-moment chunk seeks remain a follow-up).
