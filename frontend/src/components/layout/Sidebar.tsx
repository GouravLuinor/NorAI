...
export function Sidebar() {
  ...
  const handleCollapse = () => {
    toggleSidebar()
    // Update the CSS variable to animate the grid
    const workspace = document.querySelector('.workspace') as HTMLElement | null
    if (workspace) {
      workspace.style.setProperty('--sidebar-width', sidebarCollapsed ? '220px' : '0px')
    }
  }

  return (
    <div
      className={`bg-ns border-r border-bdr flex flex-col overflow-hidden transition-opacity duration-240 ${
